...
def loadRobot ():
    ...
    # Section for making the robot to change its morphology due to the noise application on the extensible joints, as well as the position of the NAO in the "z" axis.
    if datah.contGen < triggen:       
        # Start Leg Extension
        extension_leg = ((FinalLength - StartLenght)  + rd.gauss(mu, sigma))
