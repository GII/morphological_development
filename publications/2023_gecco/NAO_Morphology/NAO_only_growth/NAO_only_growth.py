...
if cluster == True:
    ...
    relativePath = 'NAO_only_growth'
    port_conexion = 19997

# Opening the connection with CoppeliaSim simulator.
print ('Program started')
sim.simxFinish(-1) # just in case, close all opened connections
clientID=sim.simxStart('127.0.0.1', port_conexion, True, True, 5000, 5) # Connect CoppeliaSim
if clientID!=-1:
...
# Parameter configuration of the NEAT algorithm.
...
def loadRobot ():
    ...
    if datah.contGen <= trigGrowth:
        ...
        if cluster == True:
            ...
            pathToScene = '/home/martin/Python_Projects/2023_GECCO/zzSourceCode/source_code/' + relativePath + nameBase
            retCode = sim.simxLoadScene(clientID, pathToScene, 1, sim.simx_opmode_blocking)
    ...
    if datah.contGen <= trigGrowth:        
        extension_leg = ((FinalLength - StartLenght) * (float(datah.contGen) / trigGrowth))
...
def ev_single_genome(genome):
    ...
    for timeStep in range(0, simSteps):
        ...
        retCode = sim.simxSetJointTargetPosition(clientID, handler.NAO_m_joint3[1],  angulo_final[0], sim.simx_opmode_oneshot)
        retCode = sim.simxSetJointTargetPosition(clientID, handler.NAO_m_joint4[1],  angulo_final[1], sim.simx_opmode_oneshot)
        retCode = sim.simxSetJointTargetPosition(clientID, handler.NAO_m_joint5[1],  angulo_final[2], sim.simx_opmode_oneshot)
        retCode = sim.simxSetJointTargetPosition(clientID, handler.NAO_m_joint7[1],  angulo_final[3], sim.simx_opmode_oneshot)
        retCode = sim.simxSetJointTargetPosition(clientID, handler.NAO_m_joint9[1],  angulo_final[4], sim.simx_opmode_oneshot)
        retCode = sim.simxSetJointTargetPosition(clientID, handler.NAO_m_joint10[1], angulo_final[5], sim.simx_opmode_oneshot)

        retCode = sim.simxSetJointTargetPosition(clientID, handler.NAO_m_joint11[1], angulo_final[6], sim.simx_opmode_oneshot)
        retCode = sim.simxSetJointTargetPosition(clientID, handler.NAO_m_joint12[1], angulo_final[7], sim.simx_opmode_oneshot)
        retCode = sim.simxSetJointTargetPosition(clientID, handler.NAO_m_joint13[1], angulo_final[8], sim.simx_opmode_oneshot)
        retCode = sim.simxSetJointTargetPosition(clientID, handler.NAO_m_joint15[1], angulo_final[9], sim.simx_opmode_oneshot)
        retCode = sim.simxSetJointTargetPosition(clientID, handler.NAO_m_joint17[1], angulo_final[10], sim.simx_opmode_oneshot)
        retCode = sim.simxSetJointTargetPosition(clientID, handler.NAO_m_joint18[1], angulo_final[11], sim.simx_opmode_oneshot)

        retCode = sim.simxSetJointTargetPosition(clientID, handler.NAO_m_joint19[1], angulo_final[12], sim.simx_opmode_oneshot)
        retCode = sim.simxSetJointTargetPosition(clientID, handler.NAO_m_joint20[1], angulo_final[13], sim.simx_opmode_oneshot)
