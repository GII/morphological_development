...
triggen = 150.0  # Generation at which the robot stop growing
...
mu = 0.0
sigma = 2.0 # Change the sigma value according to the experiment.
...
def loadRobot ():
    ...
    if datah.contGen <= triggen:
        pass
    elif datah.contGen > triggen and datah.triggen == False:
        if cluster == True:
            datah.triggen = True
            ...
            retCode = sim.simxLoadScene(clientID, pathToScene, 1, sim.simx_opmode_blocking)
    elif datah.contGen > triggen and datah.triggen == True:
        pass
    ...
    if datah.contGen <= triggen:        
        extension_leg = ((FinalLength - StartLenght) * (float(datah.contGen) / triggen))
...
def ev_single_genome(genome):
    ...
    for timeStep in range(0, simSteps):
        inputs_already_norm = []
        # Code section where the gaussian perturbation is added. Once the noise limit generation y surpassed, there is not noise at any input.
        if datah.contGen < triggen:
            # Value of the random gauss noise obtained for this iteration.
            aux = rd.gauss(mu, sigma)
            # Building the array of the input values to the NeuralNetwork.
            inputs_already_norm.append(A*np.sin(omega * timeStep) + aux)
            inputs_already_norm.append(A*np.sin(omega * timeStep + np.pi/2.0) + aux)
            inputs_already_norm.append(A*np.sin(omega * timeStep + np.pi/3.0) + aux)
        else:
