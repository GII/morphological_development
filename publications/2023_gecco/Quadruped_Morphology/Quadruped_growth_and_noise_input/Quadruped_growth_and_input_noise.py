...
# Whther the CESGA is not used, select the port '19997' by default.
else:
    port_conexion = 19997
...
triggen = 60            # Generation at which growth ends.
maxLength = 0.075       # Maximum extension, growth, of the robot legs (m).
startPos = 0.089        # Initial position of the robot in the "z" axis.
...
A = 2.0
omega = 1.0         # Change the sigma value according to the experiment

# Parameters of the gaussian function utilized for generation random noise.
mu = 0.0
sigma = 0.3 # Change the sigma value according to the experiment
...
def loadRobot ():
    ...
    if datah.contGen < triggen:
        finalLenght = maxLength * (float(datah.contGen) / triggen)
    else:
        finalLenght = maxLength
    
    handler.model = sim.simxGetObjectHandle(clientID, 'Body', sim.simx_opmode_blocking)
    
    # Set the position of the robot in the "z" according to the growth of the legs.
    finalPos = [0.0, 0.0, cp.deepcopy(startPos + finalLenght)]
    retCode = sim.simxSetObjectPosition(clientID, handler.model[1], -1, finalPos, sim.simx_opmode_oneshot)
    ...
    # Section where the joints of the robot are set to the corresponding lenght for the current generation.
    retCode = sim.simxSetJointTargetPosition(clientID, handler.jd1e[1], finalLenght, sim.simx_opmode_oneshot)
    retCode = sim.simxSetJointTargetPosition(clientID, handler.jd3e[1], finalLenght, sim.simx_opmode_oneshot)
    retCode = sim.simxSetJointTargetPosition(clientID, handler.ji1e[1], finalLenght, sim.simx_opmode_oneshot)
    retCode = sim.simxSetJointTargetPosition(clientID, handler.ji3e[1], finalLenght, sim.simx_opmode_oneshot)
    retCode = sim.simxSetJointPosition(clientID, handler.jd1e[1], finalLenght, sim.simx_opmode_oneshot)
    retCode = sim.simxSetJointPosition(clientID, handler.jd3e[1], finalLenght, sim.simx_opmode_oneshot)
    retCode = sim.simxSetJointPosition(clientID, handler.ji1e[1], finalLenght, sim.simx_opmode_oneshot)
    retCode = sim.simxSetJointPosition(clientID, handler.ji3e[1], finalLenght, sim.simx_opmode_oneshot)
...
def ev_single_genome(genome):
    ...
    for timeStep in range(0, simSteps):
        ...
        # Addition of gaussian noise at the inputs of the NeuralNetwork until the specific generation
        if datah.contGen < triggen:
            inputs_already_norm.append(A*np.sin(omega * timeStep) + rd.gauss(mu, sigma))
        else:
        ...
        # Code section that implement the random noise at the output of the NeuralNetwork.
        if datah.contGen < triggen:
            for m in range(0, len(final_angle)):
                final_angle[m] = final_angle[m] + rd.gauss(mu, sigma)
        else:
            pass
