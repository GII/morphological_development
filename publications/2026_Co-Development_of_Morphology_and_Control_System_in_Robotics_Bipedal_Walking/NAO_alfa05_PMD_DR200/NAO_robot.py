...
start_generation_DR = 200
end_generation_DR = 250
percentage_leg_variation = 0.05
percentage_body_variation = 0.05
percentage_gravity_variation = 0.01
...
mass_upper_base = 0.7700000405311584
mass_NAO = 0.7700000405311584
mass_wheel5 = 0.45868802070617676
mass_wheel6 = 0.45868802070617676
mass_wheel7 = 0.19200000166893005
mass_wheel8 = 0.2157600075006485
mass_wheel13 = 0.45868802070617676
mass_wheel14 = 0.45868802070617676
mass_wheel15 = 0.19200000166893005
mass_wheel16 = 0.2157600075006485
...
class handlers ():
    def __init__(self):
        ...
        self.shape_wheel7 = 0
        self.shape_wheel8 = 0
        self.shape_wheel13 = 0
        self.shape_wheel14 = 0
        self.shape_wheel15 = 0
        self.shape_wheel16 = 0
...
def loadRobot (port_con):
    ...
    handler.NAO_body_extension = datah.sim.getObject("/extend_body")

    handler.NAO = datah.sim.getObject("/NAO")
    handler.head = datah.sim.getObject("/imported_part_16_sub0")

    if datah.contGen <= trig45:        

        # Start Leg Extension
        extension_leg = ((FinalLength - StartLenght) * (float(datah.contGen) / trig45))
        datah.finalLenght = StartLenght + extension_leg
            
        movement_in_z = abs(extension_leg * (np.sin(75.1*np.pi / 180) + np.sin(59.0*np.pi / 180)))
        finalPos = [0.0, 0.0, startPos + movement_in_z]
                
        datah.sim.setObjectPosition(handler.NAO, datah.sim.handle_world, finalPos)
        
        datah.sim.setJointTargetPosition(handler.NAO_m_joint6,  -datah.finalLenght)
        datah.sim.setJointTargetPosition(handler.NAO_m_joint8,  -datah.finalLenght)
        datah.sim.setJointTargetPosition(handler.NAO_m_joint14,  -datah.finalLenght)
        datah.sim.setJointTargetPosition(handler.NAO_m_joint16,  -datah.finalLenght)
        datah.sim.setJointTargetPosition(handler.NAO_body_extension, datah.finalLenght)
        datah.sim.setJointPosition(handler.NAO_m_joint6,  -datah.finalLenght)
        datah.sim.setJointPosition(handler.NAO_m_joint8,  -datah.finalLenght)
        datah.sim.setJointPosition(handler.NAO_m_joint14,  -datah.finalLenght)
        datah.sim.setJointPosition(handler.NAO_m_joint16,  -datah.finalLenght)
        datah.sim.setJointPosition(handler.NAO_body_extension,  datah.finalLenght) 

    if datah.contGen > start_generation_DR and datah.contGen < end_generation_DR:
        handler.shape_upper_base = datah.sim.getObject("/Upper_base")
        handler.NAO = datah.sim.getObject("/NAO")
        handler.shape_wheel5 = datah.sim.getObject("/NAO_wheel5")
        handler.shape_wheel6 = datah.sim.getObject("/NAO_wheel6")
        handler.shape_wheel7 = datah.sim.getObject("/NAO_wheel7")
        handler.shape_wheel8 = datah.sim.getObject("/NAO_wheel8")
        handler.shape_wheel13 = datah.sim.getObject("/NAO_wheel13")
        handler.shape_wheel14 = datah.sim.getObject("/NAO_wheel14")
        handler.shape_wheel15 = datah.sim.getObject("/NAO_wheel15")
        handler.shape_wheel16 = datah.sim.getObject("/NAO_wheel16")

        datah.sim.setShapeMass(handler.shape_upper_base, mass_upper_base*(1.0 + rd.uniform(-percentage_body_variation, percentage_body_variation)))
        datah.sim.setShapeMass(handler.NAO, mass_NAO*(1.0 + rd.uniform(-percentage_body_variation, percentage_body_variation)))
        datah.sim.setShapeMass(handler.shape_wheel5, mass_wheel5*(1.0 + rd.uniform(-percentage_leg_variation, percentage_leg_variation)))
        datah.sim.setShapeMass(handler.shape_wheel6, mass_wheel6*(1.0 + rd.uniform(-percentage_leg_variation, percentage_leg_variation)))
        datah.sim.setShapeMass(handler.shape_wheel7, mass_wheel7*(1.0 + rd.uniform(-percentage_leg_variation, percentage_leg_variation)))
        datah.sim.setShapeMass(handler.shape_wheel8, mass_wheel8*(1.0 + rd.uniform(-percentage_leg_variation, percentage_leg_variation)))
        datah.sim.setShapeMass(handler.shape_wheel13, mass_wheel13*(1.0 + rd.uniform(-percentage_leg_variation, percentage_leg_variation)))
        datah.sim.setShapeMass(handler.shape_wheel14, mass_wheel14*(1.0 + rd.uniform(-percentage_leg_variation, percentage_leg_variation)))
        datah.sim.setShapeMass(handler.shape_wheel15, mass_wheel5*(1.0 + rd.uniform(-percentage_leg_variation, percentage_leg_variation)))
        datah.sim.setShapeMass(handler.shape_wheel16, mass_wheel6*(1.0 + rd.uniform(-percentage_leg_variation, percentage_leg_variation)))

        datah.sim.setArrayParam(datah.sim.arrayparam_gravity, [0.0, 0.0, -9.81*(1.0 + rd.uniform(-percentage_gravity_variation, percentage_gravity_variation))])


    if datah.contGen >= end_generation_DR and datah.contGen < end_generation_DR+2:
        handler.shape_upper_base = datah.sim.getObject("/Upper_base")
        handler.NAO = datah.sim.getObject("/NAO")
        handler.shape_wheel5 = datah.sim.getObject("/NAO_wheel5")
        handler.shape_wheel6 = datah.sim.getObject("/NAO_wheel6")
        ...
        datah.sim.setShapeMass(handler.shape_upper_base, mass_upper_base)
        datah.sim.setShapeMass(handler.NAO, mass_NAO)
        datah.sim.setShapeMass(handler.shape_wheel5, mass_wheel5)
        datah.sim.setShapeMass(handler.shape_wheel6, mass_wheel6)
        datah.sim.setShapeMass(handler.shape_wheel7, mass_wheel7)
        datah.sim.setShapeMass(handler.shape_wheel8, mass_wheel8)
        datah.sim.setShapeMass(handler.shape_wheel13, mass_wheel13)
        datah.sim.setShapeMass(handler.shape_wheel14, mass_wheel14)
        datah.sim.setShapeMass(handler.shape_wheel15, mass_wheel5)
        datah.sim.setShapeMass(handler.shape_wheel16, mass_wheel6)
        datah.sim.setArrayParam(datah.sim.arrayparam_gravity, [0.0, 0.0, -9.81])
...
def ev_single_genome(various_inputs):
    ...
    if early_trigger == True:
        falls = 1.0
        fitnessA = ((timeStep - fall_penalization)/tiempoSeg) * alfa
    else:
        falls = 0.0
        fitnessA = (timeStep/tiempoSeg) * alfa
    ...
    true_fitness = round(fitnessA + distanceA, 4)
    return [true_fitness, falls, timeStep, position[0]]
# ----------------------------------------------------------------------------------------------------------------------
...
def run_population_evolution (genIndex):
    ...
    for generation in range(0, generations):
        ...
        for m in range(0, len(fit_fall_list)):
            ...
            distance_traveled.append(fit_fall_list[m][3])
        ...
        if datah.contGen % 5 == 0 or datah.contGen == 299:
            text01 = datah.directory + '/' + 'fitness_' + str(generation) + '.txt'
            file = open(text01, 'wb')
            pickle.dump(fitness_list, file)
            file.close()
            text01 = datah.directory + '/' + 'distance_' + str(generation) + '.txt'
            file = open(text01, 'wb')
            pickle.dump(distance_traveled, file)
            file.close()
            text01 = datah.directory + '/' + 'timestep_' + str(generation) + '.txt'
            file = open(text01, 'wb')
            pickle.dump(timeStep_array, file)
            file.close()
            text01 = datah.directory + '/' + 'falls_' + str(generation) + '.txt'
            file = open(text01, 'wb')
            pickle.dump(falls, file)
            file.close()
            text01 = datah.directory + '/' + 'population_' + str(generation) + '.txt'
            file = open(text01, 'wb')
            pickle.dump(genome_list, file)
            file.close()
        else:
            pass
        ...
        datah.distance_traveled_of_the_best[datah.contGen] = distance_traveled[fit_index]
...
def main():
    ...
    coef = np.pi / 180.0
    #                        LHipYawPitch3     ,        LHipRoll3       ,      LHipPitch3        ,     LKneePitch3        ,      LAnklePitch3       ,     LAnkleRoll3        ,      RHipYawPitch3     ,        RHipRoll3       ,       RHipPitch3       ,      RKneePitch3      ,     RAnklePitch3      ,      RAnkleRoll3      ,   LShoulderPitch3      ,   RShoulderPitch3
    ...
    datah.locked = [-10.0*coef,  10.0*coef,  -10.0*coef,  10.0*coef, -50.0*coef,     10.0*coef,  48.52*coef, 48.52*coef,  -30.0*coef, -30.0*coef, 1.0*coef, 1.0*coef,  -10.0*coef,  10.0*coef,  -10.0*coef,  10.0*coef, -50.0*coef,     10.0*coef, 48.52*coef, 48.52*coef, -30.0*coef, -30.0*coef, 1.0*coef, 1.0*coef,  0.0*coef,  0.0*coef,  0.0*coef, 0.0*coef]
