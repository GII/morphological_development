...
def deNormalizeOuputs(dato_out, clase):
    ...
    if clase.contGen < triggen:
        for n in range(0, len(clase.output_range)):
            if n == 0 or n == 1 or n == 4 or n == 5 or n == 8 or n == 9 or n == 12 or n == 13:
                rango.append(clase.output_range[n])
            else:
                rango.append(clase.ang_reduced[n] + ((clase.output_range[n] - clase.ang_reduced[n]) / triggen) * clase.contGen)
    else:
