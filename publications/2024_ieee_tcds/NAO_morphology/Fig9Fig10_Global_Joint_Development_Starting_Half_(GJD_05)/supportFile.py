...
def normalizeOuputs(dato_out, clase):
    ...
    if clase.contGen < triggen:
        rango = [(((clase.locked[n]+clase.output_range[n])/2.0) + clase.contGen * ((clase.output_range[n] - ((clase.locked[n]+clase.output_range[n])/2.0)) / triggen)) for n in range(0, len(clase.output_range))]
    else:
